...
export const SCALE: {
  ...
  valid_tokens: Object.fromEntries([
    ...Array.from({ length: GRANULARITY }, (_, i) => [String.fromCharCode(65 + i), GRANULARITY - i] as const),
    ...Array.from({ length: GRANULARITY }, (_, i) => [String.fromCharCode(97 + i), GRANULARITY - i] as const),
  ]),
}
...
function toCriterion(raw: string | { id?: string; name?: string; description?: string }): Criterion {
  ...
  if (name.length === 0) throw new Error('criteria entries must declare a name or id')
 ...
 */
export function normalizeCriteria(input: CriteriaInput): Criterion[] {
  ...
  if (criteria.length === 0) throw new Error('criteria must contain at least one criterion')
  return criteria
}
...
function findTagLogprobs(
  ...
  if (!tokens || !positionLogprobs || tokens.length === 0 || positionLogprobs.length === 0) return undefined
  for (const suffix of [tag, tag.slice(0, -1)]) {
    let found: LogprobToken[] | undefined
    let textSoFar = ''
...
function valueFromToken(token: string): number | undefined {
  let tok = token.trim()
  if (tok.startsWith('>')) tok = tok.slice(1).trim() // fused '>A' tokens
