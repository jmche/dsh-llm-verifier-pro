...
export async function collectRollout(stream, label = 'rollout') {
    const chunks = [];
    ...
    if (!usable) {
        ...
        const failureNote = finish.kind === 'error' && failure
            ...
            : '';
        console.error(`[bo-n] ${label} unusable: text=${String(text.length)} chars, toolCall=${String(hasToolCall)}, finish=${String(finish.kind)}${failureNote}`);
    }
...
export async function* orchestrate(deps, config, options, next) {
    ...
    try {
        ...
        const collectCapped = (label, stream) => {
            let finished = false;
            return Promise.race([
                ...
                new Promise((resolve) => {
                    const id = setTimeout(() => {
                        if (!finished)
                            console.error(`[bo-n] ${label} exceeded the ${config.timeoutMs}ms sampling budget`);
                        resolve(undefined);
                    }, config.timeoutMs);
                    id.unref?.();
                }),
        ...
        if (anchor.usable) {
            ...
            if (config.samplingMode === 'serial') {
                ...
                const inflight = [Promise.resolve(anchor)];
                for (const request of sampled) {
                    if (!request)
                        continue;
