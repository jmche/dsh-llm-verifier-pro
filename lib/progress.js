...
export const LETTER_TO_VALUE = Object.fromEntries([
    ...LETTERS.map((c, i) => [c, i / (GRANULARITY - 1)]),
    ...LETTERS.map((c, i) => [c.toLowerCase(), i / (GRANULARITY - 1)]),
]);
 ...
 */
export function buildProgressPrompt(problem, trajectoryText, nSteps, checkpointSteps) {
    ...
    out.push('');
    out.push('Score each checkpoint INDEPENDENTLY based on the agent\'s current ' +
...
export function defaultCheckpoints(totalSteps) {
    if (totalSteps === 0)
        throw new Error('need at least one step');
    if (totalSteps > 2)
        return Array.from({ length: totalSteps - 2 }, (_, i) => i + 2);
    return Array.from({ length: totalSteps }, (_, i) => i + 1);
}
/** Validate 1-indexed checkpoint steps against a trajectory length. */
export function validateCheckpoints(checkpoints, totalSteps) {
    const bad = checkpoints.filter((k) => k < 1 || k > totalSteps);
    if (bad.length > 0)
        throw new Error(`checkpoint_steps out of range 1..${totalSteps}: ${bad.join(', ')}`);
    return [...checkpoints];
}
/**
 ...
 */
function expectedValueFromAlts(alts) {
    if (!alts)
        return undefined;
    const valsToLp = new Map();
 ...
 */
export function extractProgressScores(text, tokens, positionLogprobs, n) {
    ...
    for (let i = 1; i <= n; i++) {
        if (scores[i - 1] === null) {
            ...
            if (m?.[1])
                scores[i - 1] = LETTER_TO_VALUE[m[1]] ?? null;
        }
    }
    if (scores.some((s) => s === null)) {
        ...
        if (bare.length === n) {
            for (let i = 0; i < n; i++) {
                if (scores[i] === null && bare[i] !== undefined)
                    scores[i] = LETTER_TO_VALUE[bare[i]];
            }
        }
    }
    return scores;
 ...
 */
export function meanScores(perRep) {
    const n = perRep[0]?.length ?? 0;
    const scores = [];
    for (let i = 0; i < n; i++) {
        const vals = perRep.map((rep) => rep[i]).filter((v) => v !== null);
