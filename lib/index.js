        ...
        try {
            ...
            for (const [sectionKey, configKey] of Object.entries(forward)) {
                if (config[configKey] !== undefined)
                    base[sectionKey] = config[configKey];
            }
 ...
 */
export function resolveBoNMode(ctx, config, sessionId, sectionReader = () => ({})) {
    const section = sectionReader();
    ...
    if (section.boN === true)
        return { enabled: true, nCandidates: globalCandidates, source: 'settings-global' };
    ...
    if (section.boN !== false && config.boN)
        return { enabled: true, nCandidates: globalCandidates, source: 'config-default' };
